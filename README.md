...
7. 重点业务和接口需要搭配单元测试，如果可以，需要尽可能多的单元测试。 
8. 使用终端展示，使用`BubbleTea`-现代 TUI 框架.Terminal UI（TUI）作为终端的GUI组件。 
9. 支持同时对多个库进行执行SQL，每一个库一个协程处理，提高并发。 
10. 在`data`文件夹中生成一个`history`文件，用于记录上一次执行哪些数据库ID。 
11. 当前工具需要支持在windows终端、mac终端、linux终端中执行。 
12. 期望配置文件`yaml`如下，按照配置文件进行开发
